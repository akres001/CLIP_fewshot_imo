...
- Move `split_alexey_cub.json`  into `$DATA/cub` from `files` folder in this repo. 
...
- Move `split_alexey_plantdoc.json`  into `$DATA/plantdoc` from `files` folder in this repo. 
...
### OpenImages
- Create a folder named `open_images_v4/` under `$DATA`.
- Download the images from [here](https://storage.googleapis.com/openimages/web/download_v7.html) and keep only the ones with image-ids in `oi_data.csv` file
